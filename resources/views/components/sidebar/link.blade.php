...
@php
    $isActiveClasses = $isActive ? 'bg-gray-200' : 'text-gray-700 hover:text-gray-700 hover:bg-gray-100';
...
@if ($collapsible)
    <button type="button" {{ $attributes->merge(['class' => $classes]) }}>
        ...
        <span x-show="isSidebarOpen || isSidebarHovered" aria-hidden="true"
            class="relative block ml-auto w-6 h-6>
            <span :class="open ? '-rotate-45' : 'rotate-45'"
                class="absolute
            right-[9px] bg-gray-400 mt-[-5px] h-2 w-[2px] top-1/2 transition-all duration-200"></span>
...
@else
    <a {{ $attributes->merge(['class' => $classes]) }}>
        ...
        <span class="text-base font-medium pt-1" x-show="isSidebarOpen || isSidebarHovered">
            {{ $title }}
        </span>
    </a>
@endif
